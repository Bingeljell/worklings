...
enum ScreenPlacementChecks {
    ...
    private static func checkDefaultOrigin(context: inout CheckContext) {
        let actual = ScreenPlacement.defaultOrigin(
            windowSize: PlacementSize(width: 100, height: 80),
            visibleFrame: PlacementRect(x: 0, y: 40, width: 1_000, height: 700),
            margin: 20
        )

        context.expectEqual(
            actual,
            PlacementPoint(x: 880, y: 60),
    ...
    private static func checkClampedOrigin(context: inout CheckContext) {
        let actual = ScreenPlacement.clampedOrigin(
            proposed: PlacementPoint(x: 900, y: -100),
            windowSize: PlacementSize(width: 180, height: 140),
            visibleFrame: PlacementRect(x: 50, y: 30, width: 800, height: 600),
            margin: 10
        )

        context.expectEqual(
            actual,
            PlacementPoint(x: 660, y: 40),
    ...
    private static func checkOversizedWindow(context: inout CheckContext) {
        let actual = ScreenPlacement.clampedOrigin(
            proposed: PlacementPoint(x: 500, y: 500),
            windowSize: PlacementSize(width: 600, height: 600),
            visibleFrame: PlacementRect(x: 100, y: 50, width: 400, height: 300),
            margin: 12
        )

        context.expectEqual(
            actual,
            PlacementPoint(x: 112, y: 62),
    ...
    private static func checkRoamingOrigin(context: inout CheckContext) {
        let actual = ScreenPlacement.roamingOrigin(
            from: PlacementPoint(x: 600, y: 300),
            ...
            windowSize: PlacementSize(width: 200, height: 200),
            visibleFrame: PlacementRect(x: 0, y: 0, width: 1_200, height: 800),
            margin: 20
        )

        context.expectEqual(
            actual,
            PlacementPoint(x: 840, y: 244),
    ...
    private static func checkRoamingReflectsAtEdge(context: inout CheckContext) {
        let actual = ScreenPlacement.roamingOrigin(
            from: PlacementPoint(x: 780, y: 120),
            intent: PetRoamingIntent(
            ...
            windowSize: PlacementSize(width: 200, height: 200),
            visibleFrame: PlacementRect(x: 0, y: 0, width: 1_000, height: 700),
            margin: 20
        )

        context.expectEqual(
            actual,
            PlacementPoint(x: 628, y: 120),
