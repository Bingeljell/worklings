...
**Forget Acquired Gear** sits beside it, also debug-only: it hands back every item down to the starter Rubber Duck, equipped, and touches nothing else — name, needs, XP, class, and family all survive. Drops are deliberately scarce (boss clears only, never a duplicate), so the whole gear loop can be earned out in four delves and then never seen again; that is right for play and useless for iterating on the [drop beat](items.md#as-built). `PetState.forgettingAcquiredItems()` is the core operation, so the reset is checked rather than hand-rolled in the app.
