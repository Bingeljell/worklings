...
The inbox monitor always runs so files never accumulate; connecting a tool is itself the opt-in (there is no separate global toggle), the same way connecting a repo opts into git watching. Before the first connection of any tool, a one-time consent dialog states which file changes, what Worklings receives (an activity kind, source, and time — never content), that it stays local, and how to disconnect. Adapters use documented lifecycle signals rather than UI scraping or unstable transcript parsing, and emit into this inbox under their own source id. The first two — Claude Code (settings.json hooks) and Codex (`[hooks]`) — ship in `scripts/adapters/`, and an in-app connector writes their configs so a user need not edit them by hand; a local-git source additionally watches connected repositories in-app. See [Activity adapters](adapters.md).
